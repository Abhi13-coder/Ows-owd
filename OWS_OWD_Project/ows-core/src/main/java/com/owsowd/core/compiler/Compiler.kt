...
class Compiler {
    ...
    fun compile(program: Program, owdProgram: Program? = null): CompileResult {
        ...
        for (f in program.functions) {
            ...
            for (s in f.body) emitStmt(s, chunk, funLocals) { n -> funLocals.getOrPut(n) { funLocals.size } }
        ...
        for (c in program.classes) {
            ...
            for (m in c.methods) {
                ...
                for (s in m.body) emitStmt(s, chunk, funLocals) { n -> funLocals.getOrPut(n) { funLocals.size } }
        ...
        for (stmt in program.statements) {
            if (stmt is WhenStmt) {
                ...
                for (s in stmt.body) emitStmt(s, chunk, locals) { ensureLocal(it) }
    ...
    private fun emitStmt(
        ...
        when (stmt) {
            ...
            is ExprStmt -> {
                ...
                // SET_PROP/SET_INDEX (see emitExpr's SetProp/IndexSet cases)
                // fully consume their 3 stack operands and push nothing back
                // — unlike every other expression opcode, which leaves
                // exactly one result value for its caller. ExprStmt's job is
                // to discard that one leftover value, but for these two
                // there isn't one: emitting POP anyway tries to pop an
                // already-empty stack and crashes at runtime. This is why
                // *any* top-level property/index assignment used as its own
                // statement (e.g. "Counter.txt = ...", the most common single
                // line in an OWD-driven event handler) blew up the VM with
                // "ArrayDeque is empty" right after correctly performing the
                // assignment — the crash came after the real work was done,
                // which is why it was easy to miss.
                if (stmt.expr !is SetProp && stmt.expr !is IndexSet) {
