...
class VM(
    ...
    private fun jsonArrayToList(arr: JSONArray): MutableList<Any?> {
        ...
        for (i in 0 until arr.length()) list.add(wrapJson(arr.get(i)))
        return list
    }
    ...
    private fun stringifyJson(v: Any?): String = when (v) {
        null -> "null"
        ...
        is List<*> -> {
            val a = JSONArray()
            v.forEach { a.put(toJsonValue(it)) }
            a.toString()
        }
